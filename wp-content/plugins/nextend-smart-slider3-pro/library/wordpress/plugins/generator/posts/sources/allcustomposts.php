...
class N2GeneratorPostsAllCustomPosts extends N2GeneratorAbstract {
    ...
    public function renderFields($form) {
        ...
        $group = new N2ElementGroup($filter, 'poststatusgroup', n2_('Posts'));
        new N2ElementFilter($group, 'poststicky', n2_('Sticky'), 0);
        $statuses = get_post_stati();
        $statuses += array(
            'any'   => 'any',
            'unset' => 'unset',
        );
        new N2ElementList($group, 'poststatus', n2_('Post status'), 'publish', array(
            'options' => $statuses
        ));
