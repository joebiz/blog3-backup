...
class N2GeneratorEventsManagerEvents extends N2GeneratorAbstract {
    ...
    public function renderFields($form) {
        ...
        $posts = new N2ElementGroup($_filter, 'events', n2_('Events'));
        new N2ElementTextarea($posts, 'exclude_ids', n2_('Exclude event IDs (One ID per line)'), '', array(
            'fieldStyle' => 'width:150px;height:150px;'
        ));

        if (is_multisite()) {
            $multiSite = new N2ElementGroup($_filter, 'multisitegroup', n2_('Multisite'));
            new N2ElementOnOff($multiSite, 'multisite', n2_('Get all multisite events'), 0, array(
                'relatedFields' => array(
                    'generatormultiorder',
                    'generatorslidecount'
                ) ));
    ...
    public function posts_orderby($orderby) {
        ...
        if (in_array($this->order[0], $post_values)) {
